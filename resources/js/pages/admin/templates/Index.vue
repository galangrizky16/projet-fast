...
const tipeBorder: Record<string, string> = {
    ...
    tabel_data: 'border-emerald-200 bg-emerald-50',
    tabel_indent: 'border-emerald-100 bg-emerald-50/50',
...
function addKomponen(tipe: SuratKomponen['type']) {
    komponen.value.push(makeDefault(tipe));
}
function removeKomponen(i: number) {
    komponen.value.splice(i, 1);
}
function moveUp(i: number) {
    if (i > 0)
        [komponen.value[i - 1], komponen.value[i]] = [
            komponen.value[i],
            komponen.value[i - 1],
        ];
}
function moveDown(i: number) {
    if (i < komponen.value.length - 1)
        [komponen.value[i], komponen.value[i + 1]] = [
            komponen.value[i + 1],
            komponen.value[i],
        ];
}

function toNumberIndex(i: string | number): number {
    return typeof i === 'number' ? i : Number(i);
}

function addRow(k: any) {
    k.rows.push({ label: '', nilai: '' });
}
function removeRow(k: any, i: string | number) {
    const idx = toNumberIndex(i);
    if (!Number.isNaN(idx)) k.rows.splice(idx, 1);
}
function addKolom(k: any) {
    k.kolom.push({ jabatan: '', nama: '', nik: '' });
}
function removeKolom(k: any, i: string | number) {
    const idx = toNumberIndex(i);
    if (!Number.isNaN(idx)) k.kolom.splice(idx, 1);
}
function addPenerima(k: any) {
    k.penerima.push('');
}
function removePenerima(k: any, i: string | number) {
    const idx = toNumberIndex(i);
    if (!Number.isNaN(idx)) k.penerima.splice(idx, 1);
}
function addTembusan(k: any) {
    k.items.push('');
}
function removeTembusan(k: any, i: string | number) {
    const idx = toNumberIndex(i);
    if (!Number.isNaN(idx)) k.items.splice(idx, 1);
}

const placeholderUmum = [
    { key: 'nomor_surat', label: 'No. Surat' },
    { key: 'nama_pemohon', label: 'Nama' },
    { key: 'nim', label: 'NIM' },
    { key: 'program_studi', label: 'Prodi' },
    { key: 'tanggal_surat_panjang', label: 'Tanggal' },
...
function parseCssLayout(cssStyle?: string | null) {
    ...
    const mt = cssStyle.match(
        /margin:\s*([\d.]+\w+)\s+([\d.]+\w+)\s+([\d.]+\w+)\s+([\d.]+\w+)/,
    );
    const bi = cssStyle.match(
        /\.surat-content\s*\{[^}]*padding-left:\s*([\d]+)px/,
    );
    const pi = cssStyle.match(
        /\.surat-paragraf\s*\{[^}]*text-indent:\s*([\d]+)px/,
    );
    const ti = cssStyle.match(
        /\.surat-tabel\s*\{[^}]*margin-left:\s*([\d]+)px/,
    );
    return {
        margin_top: mt?.[1] ?? '',
        margin_right: mt?.[2] ?? '',
...
const form = useForm({
    ...
    approval_role_id:
        props.selectedJenisSurat?.approval_role?.id ?? ('' as any),
    ...
    field_config: [
        ...(props.selectedJenisSurat?.field_config ?? []),
    ] as FieldConfig[],
    layout: {
        margin_top: '',
        margin_right: '',
...
const komponen = ref<SuratKomponen[]>(
    parseKomponen(props.selectedJenisSurat?.template?.template_body),
);
...
watch(
    ...
    (sel) => {
        ...
        Object.assign(
            layout,
            parseCssLayout((sel?.template as any)?.css_style),
        );
    },
); //{ immediate: true });
...
const fieldTypeOptions = [
    ...
    { value: 'recipient', label: 'Kepada Yth.' },
];
...
function saveTemplate() {
    ...
    router.put(
        `${rolePrefix.value}/templates/${props.selectedJenisSurat.id}`,
        {
...
function deleteTemplate() {
    if (!props.selectedJenisSurat) return;
    if (!confirm(`Hapus jenis surat "${props.selectedJenisSurat.nama}"?`))
        return;
    router.delete(
        `${rolePrefix.value}/templates/${props.selectedJenisSurat.id}`,
        {
            onSuccess: () => router.visit(`${rolePrefix.value}/templates`),
        },
    );
}
...
function submitAdd() {
    addForm.post(`${rolePrefix.value}/templates`, {
        onSuccess: () => {
...
function saveGlobalSettings() {
    router.post(
        `${rolePrefix.value}/settings/template`,
...
function toggleActive(id: number, nama: string, current: boolean) {
    if (confirm(`${current ? 'Nonaktifkan' : 'Aktifkan'} "${nama}"?`)) {
        router.patch(
            `${rolePrefix.value}/templates/${id}/toggle-active`,
            {},
            { preserveScroll: true },
        );
    }
...
function duplicate(id: number) {
    if (confirm('Duplikat jenis surat ini beserta semua isinya?')) {
        router.post(`${rolePrefix.value}/templates/${id}/duplicate`);
    }
}
...
function settingLabel(key: string): string {
    ...
    return (
        props.globalSettings.find((s) => s.key === key)?.label ?? m[key] ?? key
    );
}
</script>
...
<template>
    ...
    >
        <template #actions>
            ...
            >
                <Settings class="size-3.5 text-slate-500" /> Pengaturan Kop &
                Footer
            </button>
            <button
                type="button"
                class="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-emerald-700"
        ...
        <div class="flex gap-5">
            <!-- Sidebar -->
            <aside
                class="sticky top-4 w-56 shrink-0 self-start overflow-hidden rounded-2xl border border-slate-200 bg-white"
            >
                <div class="space-y-2 border-b border-slate-100 px-4 py-3">
                    ...
                    <p class="text-[10px] text-slate-500">
                        {{ filteredJenisSurats.length }} tersedia
                    </p>
                    <div class="relative">
                        <Search
                        ...
                        <input
                            ...
                            type="text"
                            placeholder="Cari..."
                            class="h-8 w-full rounded-lg border border-slate-200 bg-slate-50 pr-2 pl-7 text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                        />
                    </div>
                </div>
                <div
                    class="max-h-[calc(100vh-220px)] divide-y divide-slate-100 overflow-y-auto"
                >
                    <Link
                        v-for="jenis in filteredJenisSurats"
                        :key="jenis.id"
                        :href="`${rolePrefix}/templates?jenis_surat_id=${jenis.id}`"
                        class="block px-4 py-3 transition-colors hover:bg-slate-50"
                        :class="
                            selectedJenisSuratId === jenis.id
                    ...
                    >
                        <div class="flex items-start justify-between gap-1">
                            <p
                                class="truncate text-xs leading-tight font-semibold text-slate-800"
                            >
                                {{ jenis.nama }}
                            </p>
                            <span
            ...
            <div class="min-w-0 flex-1">
                <div
                    ...
                    class="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center"
                >
                    <p class="text-sm text-slate-500">
                        Pilih jenis surat dari sidebar kiri.
                    </p>
                    <button
                        type="button"
                        class="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700"
                        @click="showAddDialog = true"
                    >
                ...
                <div v-else class="space-y-4">
                    ...
                    >
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p
                                    v-if="selectedJenisSurat.kode_surat"
                                    class="font-mono text-xs text-slate-500"
                                >
                                    {{ selectedJenisSurat.kode_surat }}
                                </p>
                            </div>
                            <div
                            ...
                            >
                                ...
                                >
                                    <Copy class="size-3.5 text-slate-500" />
                                    Duplikat
                                </button>
                                <button
                                    type="button"
                                ...
                                <button
                                    type="button"
                                    class="flex items-center gap-1 rounded-xl border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-100"
                                ...
                                <button
                                    type="button"
                                    class="flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50"
                                    @click="
                                        router.visit(`${rolePrefix}/templates`)
                                    "
                                >
                        ...
                        <div
                            class="mt-4 flex gap-1 border-t border-slate-100 pt-4"
                        >
                            <button
                                v-for="tab in [
                                    'template',
                    ...
                    <div v-if="activeTab === 'template'" class="space-y-4">
                        ...
                        >
                            ...
                            <p class="mt-0.5">
                                Buat isi surat di bawah. Kop dan footer akan
                        ...
                        >
                            <p
                                class="mb-3 text-xs font-semibold text-slate-700"
                            >
                                Tambah Komponen
                            </p>
                            <div class="space-y-2">
                                ...
                                >
                                    <p
                                        class="mb-1.5 text-[10px] font-semibold tracking-wider text-slate-400 uppercase"
                                    >
                                        {{ group.label }}
                                    </p>
                                    <div class="flex flex-wrap gap-1.5">
                                        <button
                                            ...
                                            :key="tipe"
                                            type="button"
                                            class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700 transition-colors hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700"
                        ...
                        >
                            <p class="text-sm text-slate-400">
                                Belum ada komponen. Tambahkan dari tombol di
                                atas.
                            </p>
                        </div>
                        ...
                        >
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <label class="space-y-1"
                                        ...
                                        <input
                                            ...
                                            type="text"
                                            class="h-8 w-full rounded-lg border border-slate-300 bg-white px-2 font-mono text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                                    ...
                                    <label class="space-y-1"
                                        ...
                                        <input
                                            ...
                                            type="text"
                                            class="h-8 w-full rounded-lg border border-slate-300 bg-white px-2 text-xs text-slate-800 outline-none focus:border-emerald-400"
                                            placeholder="-"
                                    /></label>
                                    <label class="space-y-1"
                                        ><span
                                        ...
                                        <input
                                            ...
                                            type="text"
                                            class="h-8 w-full rounded-lg border border-slate-300 bg-white px-2 text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                            ...
                            <!-- JUDUL -->
                            <div
                                v-else-if="komp.type === 'judul'"
                                class="space-y-2"
                            >
                                <input
                                    v-model="(komp as any).teks"
                                    type="text"
                                    class="h-10 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm font-bold text-slate-900 uppercase placeholder-slate-400 outline-none focus:border-emerald-400"
                            ...
                            >
                                ...
                                <div class="flex flex-wrap items-center gap-3">
                                    ...
                                    <div class="flex flex-wrap gap-1">
                                        <button
                                            v-for="p in placeholderUmum.slice(
                                                0,
                                                5,
                                            )"
                                            :key="p.key"
                                            type="button"
                                            class="rounded-lg border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 font-mono text-[10px] text-emerald-700 hover:bg-emerald-100"
                            ...
                            >
                                ...
                                >
                                    <input
                                        v-model="row.label"
                                        type="text"
                                        class="h-8 w-32 rounded-lg border border-slate-300 bg-white px-2 text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                                        placeholder="Label"
                                    />
                                    ...
                                    <input
                                        v-model="row.nilai"
                                        type="text"
                                        class="h-8 flex-1 rounded-lg border border-slate-300 bg-white px-2 font-mono text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                                        placeholder="Nilai atau {{placeholder}}"
                                    />
                                    <button
                                        type="button"
                                        class="text-slate-400 hover:text-red-500"
                            ...
                            >
                                ...
                                >
                                    <span
                                        class="w-5 text-center text-xs text-slate-500"
                                        >{{ Number(ti) + 1 }}.</span
                                    >
                                    <input
                            ...
                            >
                                ...
                                <input
                                    ...
                                    min="4"
                                    max="120"
                                    class="h-8 w-20 rounded-lg border border-slate-300 bg-white px-2 text-xs text-slate-800 outline-none focus:border-emerald-400"
                                />
                        ...
                        >
                            <div>
                                ...
                                <p class="mt-0.5 text-xs text-slate-400">
                                    Atur margin halaman dan indent konten khusus
                                    untuk template ini
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-[10px] text-slate-400">
                                    Indent body menambah padding kiri & kanan
                                    seluruh konten surat
                                </p>
                            </div>
                        </div>
                        ...
                        >
                            ...
                            >
                                <Eye class="size-3.5 text-slate-500" /> Preview
                                PDF
                            </a>
                            <div class="ml-auto">
                                <button
                                    type="button"
                                    class="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-5 py-2 text-xs font-semibold text-white transition-colors hover:bg-emerald-700"
                                    @click="saveTemplate"
                                >
                    ...
                    >
                        <div class="mb-4 flex items-center justify-between">
                            ...
                            <button
                                type="button"
                                class="flex items-center gap-1.5 rounded-xl bg-emerald-100 px-3 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-200"
                                @click="addField"
                            >
                        ...
                        <div class="space-y-3">
                            ...
                            >
                                ...
                                >
                                    <code
                                        class="rounded-lg bg-emerald-100 px-2 py-0.5 font-mono text-[10px] text-emerald-800"
                                        >&#123;&#123;{{
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <label class="space-y-1"
                                        ...
                                        <select
                                            v-model="field.type"
                                            class="h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-xs text-slate-800 outline-none focus:border-emerald-400"
                                        >
                                            <option
                        ...
                        >
                            <button
                                type="button"
                                class="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700"
                                @click="saveTemplate"
                            >
                    ...
                    >
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <label class="space-y-1.5"
                                ...
                                >
                                <input
                                    v-model="form.kode_surat"
                                    type="text"
                                    class="h-10 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 font-mono text-sm text-slate-800 uppercase placeholder-slate-400 outline-none focus:border-emerald-400"
                                    placeholder="CUTI-MHS"
                            /></label>
                            <label class="space-y-1.5"
                                ><span
                                    class="text-xs font-medium text-slate-700"
                                ...
                                <select
                                    ...
                                    class="h-10 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 text-sm text-slate-800 outline-none focus:border-emerald-400"
                                >
                ...
                >
                    <label class="block space-y-1.5"
                        ...
                        <input
                            ...
                            type="text"
                            required
                            class="h-10 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 text-sm text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="space-y-1.5"
                            ...
                            >
                            <input
                                v-model="addForm.kode_surat"
                                type="text"
                                class="h-10 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 font-mono text-sm text-slate-800 uppercase placeholder-slate-400 outline-none focus:border-emerald-400"
                                placeholder="UD-YUDIS"
                        /></label>
                        <label class="space-y-1.5"
                            ><span class="text-xs font-medium text-slate-700"
                                >Kategori</span
        ...
        <Dialog
            ...
            @update:open="(v) => !v && (showGlobalSettings = false)"
        >
            <DialogContent
            ...
            >
                ...
                <div class="space-y-4 px-6 py-5">
                    ...
                    >
                        ...
                        <p class="text-[10px] text-blue-600">
                            💡 "Nama Instansi" tampil di baris pertama kop
                            (misal: UNUGHA CILACAP). "Singkatan" tampil dalam
                            kurung setelah nama fakultas — kosongkan jika tidak
                            perlu.
                        </p>
                    </div>
                    ...
                    >
                        ...
                        <p class="text-[10px] text-emerald-600">
                            💡 "Nama Instansi (Footer)" bisa berbeda dari kop —
                            misal nama lengkap universitas di footer. Kosongkan
                            untuk mengikuti Nama Instansi kop.
                        </p>
                    </div>

                    <!-- TAMPILAN -->
                    <div
                        class="space-y-3 rounded-xl border border-purple-200 bg-purple-50 p-4"
                    >
                    ...
                    >
                        ...
                        >
                            🛠 HTML Kop & Footer Kustom (lanjutan)
                        </summary>
                        <div class="space-y-3 px-4 pb-4">
                            <p class="text-[10px] text-slate-500">
                                Kosongkan untuk menggunakan kop/footer otomatis
                                dari pengaturan di atas.
                            </p>
                            <label
                            ...
                            >
                                ...
                                <textarea
                                    ...
                                    rows="4"
                                    class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 font-mono text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-emerald-400"
                ...
                >
                    <Button
                        ...
                        variant="outline"
                        class="rounded-xl text-slate-700"
                        @click="showGlobalSettings = false"
                        >Batal</Button
                    >
                    <Button
                        type="button"
